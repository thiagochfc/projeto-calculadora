...
namespace projeto_calculadora.View
{
    public partial class FrmPrincipal : Form
    {
        ...
        {
            if (controller.VeriricaSeIgualPressionado(_PressionouIgual))
            {
                LimparTxtResultado();
                _PressionouIgual = false;
            }
            if (controller.VerificaSeIgualZero(TxtResultado)) TxtResultado.Text += valor;
        ...
        {
            if (!controller.VerificaSeVazio(TxtResultado))
            {
                if (controller.VerificaSeTemPonto(TxtResultado)) _NumeroUm = Convert.ToDouble(TxtResultado.Text.Trim().Replace(".", ","));
        ...
        private void BtnIgual_Click(object sender, EventArgs e)
        {
            ...
            if (!controller.VerificaSeVazio(TxtResultado))
            {
                ...
                if (controller.VerificaSeTemPonto(TxtResultado)) _NumeroDois = Convert.ToDouble(TxtResultado.Text.Trim().Replace(".", ","));
        ...
        private void BtnPonto_Click(object sender, EventArgs e)
        {
            ...
            if (controller.VerificaSeVazio(TxtResultado)) TxtResultado.Text = "0.";
            if (controller.VerificaSeTemPonto(TxtResultado)) return;
            TxtResultado.Text += ".";
            PnlFundo.Focus();
        }
        ...
        {
            if (!controller.VerificaSeVazio(TxtResultado)) TxtResultado.Text = (Convert.ToDouble(TxtResultado.Text.Trim().Replace(".", ",")) * (-1)).ToString().Replace(",", ".");
            PnlFundo.Focus();
        }
